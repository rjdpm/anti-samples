...
def generate_antisamples(unlearn_data,
             retain_data,
             retain_labels=0,
             local_variance = 10,
             size_multiplier = 1,
    ...
    ## Transfering Tensor to Numpy
    unlearn_data = unlearn_data.numpy() if isinstance(unlearn_data, (torch.Tensor)) else unlearn_data
    retain_data = retain_data.numpy() if isinstance(retain_data, (torch.Tensor)) else retain_data
    retain_labels = retain_labels.numpy() if isinstance(retain_labels, (torch.Tensor)) else retain_labels
    
    print(f'Number of unlearn data = {len(unlearn_data)}')
    print(f'Number of retain data = {len(retain_data)}')
    flag1 = time.time()
    
    pairwise_distance = pairwise_distances(unlearn_data, retain_data)
    
    flag2 = time.time()
    
    antisamples = np.zeros((unlearn_data.shape[0]*size_multiplier, unlearn_data.shape[1]))
    ...
    for epoch in tqdm(range(size_multiplier)):
        ...
        antisamples[epoch*unlearn_data.shape[0]:(epoch + 1)*unlearn_data.shape[0]] = anti_data
        
    if convex_combination == True:        
        temp_matrix = np.random.normal(loc=0, scale=1, size=(no_generated_data, antisamples.shape[0]))
        exp_matrix = np.exp(temp_matrix)
        softmax_values = exp_matrix / np.sum(exp_matrix, axis=1, keepdims=True)
        antisamples = np.dot(softmax_values, np.array(antisamples))
        
    else:
        antisamples = antisamples
    
    flag4 = time.time()
    unmunge_time = flag4 - flag1
    print(f'Number of Generated Data = {len(antisamples)}')
    ...
    return torch.from_numpy(antisamples), farthest_point, pairwise_distance, unmunge_time
