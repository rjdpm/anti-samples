...
if not os.path.isfile(random_classes_filename):
    all_unlearn_classes = [None]*len(all_averaging_epochs_list)
...
print(f'List of all unlearn classes are:\n {all_unlearn_classes}')
print('-'*80)
print('='*80)
...
for multiclass_idx in range(len(num_multiclass_list)):
    ...
    for epoch in tqdm(range(averaging_epochs)):
        ...
        for cls in unlearn_classes:
            idx_train = retain_train_labels == cls
            idx_test = retain_test_labels == cls
        ...
        obj_model.forget_loader = DataLoader(list(zip(np.concatenate(unlearn_data),np.concatenate(unlearn_labels))), batch_size=batch_size, shuffle=True)
        ...
        if not os.path.isfile(obj_model.best_unlearn_model_save_path):
            ...
            for data_idx in range(len(unlearn_data)):
                ...
                unmunge, farthest_point, pairwise_distance, unmunge_time = UNMUNGE_(unlearn_data= data,
                                                                    ...
                                                                    eps=eps,#0.01
                                                                    convex_combination=convex_combination#True
                                                                    )
