...
np_seed = 20#config['np_seed']
torch_seed = 100#config['torch_seed']
np.random.seed(np_seed)#60#20#30
torch.manual_seed(torch_seed)
random.seed(30)

# Ensure reproducibility in data loading
def seed_worker(worker_id):
    worker_seed = torch.initial_seed() % 2**32
    np.random.seed(worker_seed)
    random.seed(worker_seed)

# If CUDA is available, set seeds for all GPUs and configure deterministic settings
