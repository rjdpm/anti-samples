...
class AllCNN_(nn.Module):
    def __init__(self, filters_percentage=1., n_channels=3, n_classes=10, dropout=False, batch_norm=True, padding = 0, dataset_name = 'mnist'):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(768 if dataset_name in ['olivetti', 'casia-webface'] else n_filter2, n_classes),
        )
...
class ResNet9_(nn.Module):
    ...
    A Residual network.
    """
    def __init__(self, n_classes = 10, num_input_channels = 3, padding = 0, dataset_name = 'mnist'):
        ...
        ## For Olivetti Dataset
        if dataset_name in ['olivetti', 'casia-webface']:
            self.fc = nn.Linear(in_features=2304, out_features=n_classes, bias=True)
        else:
